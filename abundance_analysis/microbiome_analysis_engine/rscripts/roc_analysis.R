...
buildModel <- function(data, xi = 3, yi = 2, cv = FALSE, family = NULL){
  ...
  if(cv){
    ...
    m <- glm(y ~ x, data = data$train, family = "binomial") # Use 'glm()' from native R to create a model of f(x) = an + b
    pp <- predict(m, newdata = data$test, type = "response") # Use the fitted model to predict and generate prediction probabilities
    
    roc.obj <- pROC::roc(data[[1]]$response, pp) # Create a ROC object
    ...
    m <- glm(y ~ x, data = data, family = "binomial") # Use 'glm()' from native R to create a model of f(x) = an + b
...
if(length(args) != 3){
  ...
  dep.var  <- args[1] # Input Number of column which is the dependent variable
  do.split <- args[2] # Either 'TRUE' or 'FALSE'. If TRUE, model is k-fold CVed.
  iter     <- args[3] # Specify how many 'folds' for CV.
...
buildModel(learning.set, cv = TRUE)

responseModel <- function(data, x = 3, y = 2, qty = 0.8, mname = "default", do.split = FALSE){
  if(do.split){
    print("Splitting the data into a train-test sections...")
    learning.set <- sampleDataset(data, percent = qty)
