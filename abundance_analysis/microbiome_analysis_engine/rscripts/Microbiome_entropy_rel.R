# setwd("./Documents/UoL/Cancer_Studies_PhD/Study_MiST4/abundance_analysis/")
# Descriptive Statistics in R
# https://statsandr.com/blog/descriptive-statistics-in-r/#introduction
# agrmt::entropy() 1.42.8
# rasterdiv::Hill() 0.2.5.2
# abdiv::simpson() 0.2.0
# hilldiv::gamma_div() 1.5.1
# dplyr::1.0.10
# xgboost::1.6.0.1
...
runPCA            <- function(x, groups = NULL, ellipse = TRUE){
  ...
  x <- x[, colSums(x != 0) > 0]
  pca <- prcomp(x, center = TRUE, scale. = TRUE)
  p   <- fviz_pca_biplot(pca, col.var="black", label=c("ind", "ind.sup", "quali", "var", "quanti.sup"), habillage=groups, 
...
run_abun_analysis <- function(data, name = "../results.xlsx"){
  ...
  microbiome.pca$biplot + theme(axis.text    = element_text(size = 15), 
...
pkgs <- c('rasterdiv', 'agrmt', 'dplyr', 'hilldiv', 'abdiv', 'ggbiplot', 'crayon', 'factoextra') # The 'hilldiv' package requires the installation of 
...
SA.data <- load_data("../asv/abun_species.relative.xlsx") # HERE IS WHERE YOU LOAD YOUR DATASET
...
run_abun_analysis(SA.data, name = "species_relative_baseline_results")

#-------------------- Boruta/XGBoost Analysis -------------------------
cat(magenta("Analysing microbiome using Boruta/XGBoost overlaping features...\n"))
boruta      <- read.csv("../aleks_boruta/bor_sel_relative.csv")
boruta.feat <- boruta$X

xgb <- readxl::read_excel("../../MesoXGBoost/outfolder/0.05/species_relative_0.08e_600i_0.5g/species_relative_0.08e_600i_0.5g_imatrix.xlsx")
...
run_abun_analysis(SA.data.sel, name = "species_relative_xgb&boruta_results")

#-------------------- Only Boruta -------------------------
cat(magenta("Analysing microbiome using Boruta features...\n"))

boruta.feat <- boruta.feat[-1]

SA.data.bor <- SA.data %>% select(SampleID,best_percentage_change,order,response,boruta.feat)
run_abun_analysis(SA.data.bor, name = "species_relative_boruta_selected_results")
...
run_abun_analysis(SA.data.xgb, name = "species_relative_xgb_selected_results")

#-------------------- Extract & Test Significant Features ----
cat(green("Analysing extracted features...\n"))

xgb.sel    <- filter(xgb, xgb$Feature %in% sel.feat)
boruta.sel <- filter(boruta, boruta$X %in% sel.feat)

SA.data.sel.ord <- SA.data.sel[order(-SA.data.sel$response),]
rownames(SA.data.sel.ord) <- NULL

genBoxPlt <- function(data){
  # i <- 1
  for(feat in names(data[,5:ncol(data)])){
    col <- select(data, feat)
    n <- unlist(strsplit(names(col), "_"))
    n <- paste(n[1],n[2])
    ...
    pdf(paste0("./",n,"_relative_boxplot.pdf"), height = 11.69, width = 8.27)
    par(cex.lab=2.0)  # y-axis
    par(cex.axis=2.0) # x-axis
    par(cex.main=2.5) # main title 
    boxplot(cbind("Responders" = col[1:11,], "Non-Responders" = col[12:nrow(col),]),main=n)
    dev.off()
    # i <- i + 1
  }
}
genBoxPlt(SA.data.sel.ord)

responders         <- filter(SA.data.sel.ord, SA.data.sel.ord$response %in% 1)
nonresponders      <- filter(SA.data.sel.ord, SA.data.sel.ord$response %in% 0)
t <- compareMeans(responders$Oscillibacter_sp., 
             nonresponders$Oscillibacter_sp.)

responders$Oscillibacter_sp.
nonresponders$Oscillibacter_sp.

groups    <- rbind("responders", "nonresponders")

oscili             <- rbind("Responders"     = mean(responders$Oscillibacter_sp.),
                            "Non-Responders" = mean(nonresponders$Oscillibacter_sp.)) 

alisti             <- rbind("Responders"     = mean(responders$Alistipes_indistinctus),
                            "Non-Responders" = mean(nonresponders$Alistipes_indistinctus))

oscilibacter.p     <- rbind(compareMeans(responders$Oscillibacter_sp., 
                                         nonresponders$Oscillibacter_sp.)$p.value,
                            NA)
alistipes.p        <- rbind(compareMeans(responders$Alistipes_indistinctus, 
                                         nonresponders$Alistipes_indistinctus)$p.value,
                            NA)

oscilibacter.sd    <- rbind(sd(responders$Oscillibacter_sp.),
                            sd(nonresponders$Oscillibacter_sp.))

alistipes.sd       <- rbind(sd(responders$Alistipes_indistinctus),
                            sd(nonresponders$Alistipes_indistinctus))

c.names   <- c("Groups", 
               "Oscilibacter sp.", "p-value", "Standard Deviation", 
               "Alistipes indistinctus", "p-value", "Standard Deviation")

sel_res <- data.frame(groups, 
                      round(oscili,4), round(oscilibacter.p,4), round(oscilibacter.sd,4), 
                      round(alisti,4), round(alistipes.p,4), round(alistipes.sd,4))

names(sel_res) <- c.names
writexl::write_xlsx(list("statistics" = sel_res,
                         "shap"       = xgb.sel,
                         "imp"        = boruta.sel ), "./significant.xlsx")
# write.xlsx(sel_res, "./.xlsx", sheetName="New_Sheet", append=TRUE)